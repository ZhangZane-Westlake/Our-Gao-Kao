...
《下一站以前》是一款高三题材视觉小说网页游戏。玩家从百日誓师开始，经历一模碰壁、错题复盘、友情边界、家庭沟通、阶段性进步、考前低谷、高考入场和录取通知书。游戏重点不是“考高分爽文”，而是让玩家在选择里看见：努力、稳定、支持、勇气和成长怎样慢慢留下来。
...
- 字体：`cn-fontsource-lxgw-marker-gothic-regular`，全局使用 `LXGW Marker Gothic`（霞鹜漫黑）
...
- 点击开始游戏后，会先进入黑色背景的开场引入，主旨句会缓慢浮现；点击进入倒计时后会清空旧局状态，重新初始化数值，并显示“第一幕 / 百日誓师，游戏开始”的独立黑场幕名引入，再平滑淡入第一幕画面。
- 每次点击选项后，页面中部会展示所选选项、选择意图和后果，然后直接接入幕间黑场，避免背景在切幕前闪现。
- 最后一项选择完成后，会先进入结尾收束段落，再展示最终结果；最终页不再显示关键词和选择回顾，而是在总结段落后展示一个动态“最终状态”。
- 游戏顶部中间固定显示“高考倒计时”，状态面板只展示状态趋势和状态条，不显示具体数值；“这一刻的我”中状态名和状态条同排显示以节省空间，选项方块显示在右侧状态栏下方，与状态卡片保持同宽对齐。
- 对话里主角以“我”显示，旁白文本不显示“旁白：”前缀；选择前的主角回应尽量保留沉默，由旁白铺情境和情绪，具体回应放到选项和选择反馈里。
- 场景叙事、对话正文和最终状态会播放轻量文字特效：先短暂出现拼音输入感字符，再逐字显示中文；对话会按行依次开始，不会多行同时打字，对话方块独占画面底部宽度，每一幕的选项方块会在当前叙事和对话完全显示后再出现在状态栏下方。
- 浏览器内使用本地 SVG 可爱光标，普通状态是浅色斜放铅笔，可点击状态会变成金色斜放铅笔。macOS 浏览器如果仍显示系统光标，通常是浏览器对 SVG cursor 的兼容限制，可把 `public/assets/cursors` 下的 SVG 替换成同名 PNG 后继续使用。
- 第 1 幕：百日誓师，选择初始路线
...
- 多结局最终状态包括“理想发挥”“稳定上岸”“压线反转”“高分但空心”“重新认识自己”“带着光继续”，每一种都用励志口吻总结这一百天留下的东西。
...
开局基础状态整体偏稳，玩家从“有些紧张但仍能调整”的状态开始，而不是从低数值压力局开始。
...
- 开始页封面：`public/assets/generated/title/title_screen.png`
- 场景图：`public/assets/generated/scenes`
- 人物图：`public/assets/generated/characters`

如果你已经在别处生成好了图片，直接复制到这些目录即可。文件名必须和资源 key 一致：

```text
public/assets/generated/scenes/scene_oath_playground.png
public/assets/generated/scenes/scene_mock_score_classroom.png
public/assets/generated/characters/protagonist_uniform_bag.png
public/assets/generated/characters/teacher_classroom.png
```

前端会自动读取这些图片；如果某张图片不存在或加载失败，会显示占位提示。当前默认不叠加人物立绘，避免未抠图的白灰棋盘格或绿幕背景破坏画面。之后如果有透明 PNG 立绘，再打开 `src/App.tsx` 里的 `should_show_character_sprites`。

开始页封面生成 prompt：

```text
写实青春校园电影感，中国高三主题视觉小说封面，16:9 横图。清晨校园操场和教学楼远景，温暖阳光，远处有一块倒计时牌写着“距离高考还有100天”，红色横幅写着“百日誓师大会”。画面前景是一名17岁中国高三男生的背影或侧后方身影，黑色短发，身形偏瘦，穿蓝白色高中运动校服外套、白色T恤、深蓝校服长裤、白色运动鞋，背黑色双肩包，站在操场边望向倒计时牌。画面左侧或中下部预留足够暗部空间，方便叠加游戏标题“下一站以前”和开始按钮。氛围温暖、克制、青春、略带迷茫但有希望。真实校园，电影感构图，不要动漫风，不要夸张表演，不要水印，不要字幕，不要乱码，不要多余文字；除“距离高考还有100天”和“百日誓师大会”外不要出现其他文字。
```
...
- 允许害怕、停顿、回避和回头看看
- 文案保持细腻、克制，贴近考生真实体验
- 开场黑场显示：“高三就像一场游戏，它记录下分数，也记录下努力、勇气、感情。”
- 最终黑场显示：“高考不是人生的终局，而是青春里认真奔跑的证明。”
